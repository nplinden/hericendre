...
Source::Source(const pugi::xml_node& sourceNode){
    ...
    if (!params.empty()){
        ...
        std::vector<std::string> intensity(params_vector.begin() + length, params_vector.end()) ;
        std::vector<std::string> energy(params_vector.begin(), params_vector.begin() + length) ;
        for (auto& e: energy){
    ...
    if (this->type_ == "mixture"){
        for (pugi::xml_node pairNode = sourceNode.child("pair"); pairNode; pairNode = pairNode.next_sibling("pair")){
